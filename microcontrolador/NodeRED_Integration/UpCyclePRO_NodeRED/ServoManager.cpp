...
bool ServoManager::begin() {
  ...
  // Apagar todos los servos al inicializar
  disableAllServos();
...
void ServoManager::disableAllServos() {
  Serial.println("🔌 Apagando todos los servos");
  
  // Apagar completamente los servos (sin PWM)
  pwm.setPWM(SERVO1_CHANNEL, 0, 0);
  pwm.setPWM(SERVO2_CHANNEL, 0, 0);
  pwm.setPWM(SERVO3_CHANNEL, 0, 0);
  
  Serial.println("✅ Todos los servos apagados");
}
